...
class Backend extends CI_Controller {
	...
	public function index() {
		...
		$mon_appoi_cnt = $this->Common_model->RawSQL(" SELECT COUNT(DISTINCT `fld_appointid`) AS mon_appoint_cnt FROM `appointments` WHERE DATE_FORMAT(`fld_adate`, '%Y-%m') = '$year_month'  AND `fld_atype` IS NULL");

		/* Get today appointment count */
		$today_appoi_cnt = $this->Common_model->RawSQL(" SELECT COUNT(DISTINCT `fld_appointid`) AS today_appoint_cnt FROM `appointments` WHERE `fld_adate` = '$today' AND `fld_atype` IS NULL ");
		...
		$today_sales = $this->Common_model->RawSQL(" SELECT SUM(`fld_ppaid`) AS `today_paid` FROM `payments` WHERE `fld_pdate` BETWEEN '".$today." 00:00:00' AND '".$today." 23:59:59' ");

		/* get this monthly revenue value */
		$monthly_sales = $this->Common_model->RawSQL(" SELECT SUM(`fld_ppaid`) as `monthly_paid` FROM `payments` WHERE DATE_FORMAT(`fld_pdate`, '%Y-%m') = '".$year_month."' ");
	
		/* get till date revenue value */
		$till_sales = $this->Common_model->RawSQL("SELECT SUM(`fld_ppaid`) as `till_paid` FROM `payments` WHERE `fld_pdate` <= '".$today."' ");
		...
		$revenueData = $this->Common_model->GetJoinDatas( 'appointments A', 'payments P', "A.fld_aid = P.fld_appid", "DATE_FORMAT(A.fld_booked_date, '%Y-%m') AS month, SUM(P.fld_ppaid) AS total_revenue", ["YEAR(A.fld_booked_date)" => date('Y')], "A.fld_atype IS NULL", "month" );
